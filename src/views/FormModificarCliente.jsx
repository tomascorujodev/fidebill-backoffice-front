        ...
        if (response) {
          switch (response.status) {
            case 200:
              ...
              response.fechaNacimiento = convertirFechaIngles(response.fechaNacimiento);
              setFormData(response);
              setMessage("");
              return;
            case 204:
              setMessage("No se puedo encontrar el cliente, verifique si no fue modificado o eliminado");
              break;
            case 401:
              setMessage("Sus credenciales expiraron, por favor, vuelva a iniciar sesion.");
              break;
            case 500:
              setMessage("Hubo un problema en el servidor. Por favor, contacte con un administrador");
              break;
            default:
              setMessage("Hubo un problema. Por favor, contacte con un administrador");
              break;
          }
          setShowModal(true);
        }
  ...
  async function handleSubmit(e) {
    ...
    if (isValid) {
      try {
        ...
        let response = await PATCH("clientes/modificarcliente", { ...formData, idCliente: id });
        if (response) {
          switch (response.status) {
            case 200:
              setMessage("El cliente " + formData.nombre + " " + formData.apellido + ", Documento: " + formData.documento + " ha sido modificado correctamente");
              setFormData({
                nombre: "",
                apellido: "",
                documento: "",
    ...
    <div className="container card p-4 mb-4">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {isLoading ? (
          ...
          <button style={{ float: "inline-end" }} type="submit" className="btn btn-warning w-25 mt-3 custom-button">
            Modificar Cliente
          </button>
        )}
      </form>
